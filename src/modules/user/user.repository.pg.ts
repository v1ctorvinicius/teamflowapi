...
import type { User, CreateUserInput, UpdateUserInput } from "./user.types.ts";
...
function mapRowToProduct(row: Record<string, unknown>): Product {
  return {
    ...
    stockCategoricalBySize:
      (row.stock_categorical_by_size as Record<string, number>) || {},
...
export class PostgresUsersRepository implements UsersRepository {
  ...
  async create(input: CreateUserInput): Promise<User> {
    const result = await this.pool.query(
      `INSERT INTO users (username, email, password_hash, name, favorite_team)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [
        input.username,
